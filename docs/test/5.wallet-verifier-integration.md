...
Important:
- Start Verifier **after** Issuer is running.
- If you restart Issuer, it may generate a new signing key (new `kid`), and previously issued credentials in the Wallet may no longer verify unless Verifier is restarted with the new issuer `jwks.json` and you re-issue the credential.
...
### Verifier returns `Invalid issuer signature over commitmentJwt`

Cause:
- The Wallet credential was issued by an Issuer key that is different from the Issuer key the Verifier currently trusts.
- This commonly happens if:
  - Issuer was restarted (new key generated), or
  - Verifier was started with an old/stale `TRUSTED_ISSUER_PUBLIC_JWK`.

Fix (fastest):

1) Ensure Issuer is running and note the current issuer JWKS:

```bash
curl -s http://localhost:8080/jwks.json
```

2) Restart Verifier using the current issuer JWKS:
...
3) In the Wallet UI, re-issue the credential (or clear wallet storage and issue again).

Quick `kid` mismatch check:
- Copy the credential’s `commitmentJwt` from the Wallet UI and decode its header to compare `kid` to `http://localhost:8080/jwks.json`.
...
jwt = sys.argv[1]
header = jwt.split('.')[0]
print(json.dumps(json.loads(b64url(header)), indent=2))
PY "<COMMITMENT_JWT>"
```

Expected:
- The `kid` in the JWT header matches the `kid` in issuer `/jwks.json`.
